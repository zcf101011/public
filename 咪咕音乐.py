...
function encrypt(constr, pwdstr) {
            var ll
            ll = CryptoJS.AES.encrypt(constr, pwdstr)
            return ll.toString()
}
...
#    print(execjs.get().name)
    ...
    jl = execjs.compile(jsdaima)

    re = jl.call('encrypt',data, pwd)
...
def rsa_encrypt(message):
    ...
    return urllib.parse.quote(cipher_text)
...
def getHeaders(c):
    headers = {
        ...
        ':method': 'GET',
        ':path': '/v3/api/music/audioPlayer/getPlayInfo?dataType=2' + c,
        'cookie': 'mg_uem_user_id_9fbe6599400e43a4a58700a822fd57f8=4246148f-dabc-4eab-9129-d0a188235ed1; migu_cookie_id=7e740ff1-1b32-4e87-8f89-cf17da53c5a5-n41595374873066; player_stop_open=0; playlist_adding=1; addplaylist_has=1; audioplayer_new=1; add_play_now=1; mg_uem_session_id_9fbe6599400e43a4a58700a822fd57f8=c3656148-0553-4d3e-a79b-3b6822545450; playlist_change=0; audioplayer_exist=1; WT_FPC=id=23f47271dceb3f3c95f1593575609316:lv=1595730782458:ss=1595730761310; audioplayer_open=1',
...
#正则匹配
def zhengze(data):
    shuju = re.findall('"playUrl":"(.*?)"', data, re.S | re.I)
    #print(''.join(shuju))
    return 'https:' + ''.join(shuju)

#请求数据和保存
def qingqiu(url, musicname1):
    header = {
        'Referer': 'https: // music.migu.cn /'
    }
    print(url)
    resp = requests.get(url,headers = header)
    total_size = int(int(resp.headers["Content-Length"]))
    print("当前歌曲大小：{} 字节大小".format(total_size))
    print('%s正在下载中...' % musicname1)
    total = 0
    with open('D:\\音乐\\'+ musicname1 + '.flac','wb') as f:
...
def zhuyao(musicid, musicname):
        s = aes('{"copyrightId":"'+str(musicid)+'","type":'+str(3)+',"auditionsFlag":0}','4ea5c508a6566e76240543f8feb06fd457777be39549c4016436afda65d2330e')
        s = str(s)
        zh = s.replace('\n', '')
        #print(zh)
        d= '&data=' + urllib.parse.quote(zh)
        a = '&secKey=' + rsa_encrypt('4ea5c508a6566e76240543f8feb06fd457777be39549c4016436afda65d2330e')
        url = 'https://music.migu.cn/v3/api/music/audioPlayer/getPlayInfo?dataType=2' + d + a

        c = d + a
        #print(url)
        sessions=requests.session()
        sessions.mount(url, HTTP20Adapter())
        r=sessions.get(url,headers=getHeaders(c))
        #请求的数据
        #print(r)
        print(r.text)

        url = zhengze(r.text)
        qingqiu(url, musicname)


def main():
    search = input('请输入歌曲或者歌手: ')
    #search = '盗将行'

    url = 'https://m.music.migu.cn/migu/remoting/scr_search_tag?rows=20&type=2&keyword=' + urllib.parse.quote(search)

    rel = requests.get(url).json()

    res = rel["musics"]
    #print(res)
    shuju2 = re.findall(".*?{'albumName': '(.*?)', 'albumId': '(.*?)'.*?, 'copyrightId': '(.*?)'.*?'songName': '(.*?)'.*?'singerName': '(.*?)'.*?}",
        str(res), re.S | re.I)
    #print(shuju2)
    j = 0
    for i in shuju2:
        ...
        songname = i[4]
        print(str(j) + ':',
             '音乐名字: '+ musicname + '         ' + '歌手: '+songname + '       ' + '专辑名: '+albumname + '       ' + '专辑id号: '+albumid + '       ' + '音乐id: '+musicid)
        list.append(musicid)
